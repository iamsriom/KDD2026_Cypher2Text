...
try:
    from data.setup.config import DEFAULT_HF_DATASET_NAME, DEFAULT_HF_TOKEN_PATH, MODELS_DIR
except ImportError:
...
def main() -> None:
    ...
    parser.add_argument("--dataset-name", type=str, default=DEFAULT_HF_DATASET_NAME, help="HuggingFace dataset (train split).")
    parser.add_argument("--idf-path", type=Path, default=MODELS_DIR / "idf_scores_text2cypher.json")
    ...
    # Reproducibility: keep random state deterministic by default.
    ...
    dl = DataLoader(ds, batch_size=args.batch_size, shuffle=True, drop_last=drop_last, generator=g)
